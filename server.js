...
app.get('/api/admin/dashboard', adminAuth, async (req, res) => {
  try {
    ...
    const pendingFulfillmentOrders = orders.filter(o => 
      o.status === 'pending fulfillment' || o.status === 'pending fulfilment'
    ).length;
    const fulfilledOrders = orders.filter(o => o.status === 'fulfilled').length;
    const pendingOrders = pendingCheckoutOrders + pendingFulfillmentOrders; // Includes both pending checkout and pending fulfillment/fulfilment
    ...
    // Only count revenue from orders that are pending fulfillment/fulfilment or fulfilled in monthly revenue (total sale price)
    orders.forEach(order => {
      if (order.date && (order.status === 'pending fulfillment' || order.status === 'pending fulfilment' || order.status === 'fulfilled')) {
...
app.put('/api/admin/orders/:orderId/status', adminAuth, async (req, res) => {
  try {
    ...
    // If changing from pending fulfillment/fulfilment to fulfilled, require delivery cost
    if ((oldStatus === 'pending fulfillment' || oldStatus === 'pending fulfilment') && status === 'fulfilled') {
